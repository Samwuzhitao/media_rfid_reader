...
class MeshStatus(QObject):
    ...
    def get_mesh_status(self):
        ...
        if min_s <= TAG_SET_TAG and min_s >= TAG_MOVE_IN:
...
class ComMaintainer(QDialog):
    def __init__(self,parent=None):
        super(ComMaintainer,self).__init__(parent)
        self.ser_list     = []
        self.monitor_dict = {}
        self.ser          = None
    ...
    def uart_cmd_decode(self,port,data):
        ...
        if data[2:4] == '02':
            ...
            #     print set_tag_count_str,
            #     log_str = log_str + ' ' + set_tag_count_str
            if data[4:8] == '3D05': # 擦除标签TAG FAIL
                self.mesh_s.update_tag_status(ser_index,TAG_SET_TAG)
                self.mesh_s.set_tag_count[ser_index] = self.mesh_s.set_tag_count[ser_index] + 1
                if self.mesh_s.set_tag_count[ser_index] >= 5:
                    self.mesh_s.update_tag_status(ser_index,TAG_SET_FAIL)
                result_str = u"擦除标签TAG FAIL"
                set_tag_count_str =  "SET_COUNT:%d" % (self.mesh_s.set_tag_count[ser_index])
                print set_tag_count_str,
                log_str = log_str + ' ' + set_tag_count_str
            if data[4:8] == '3D01': # 擦除标签TAG FAIL
                self.mesh_s.update_tag_status(ser_index,TAG_SET_OK)
                result_str = u"擦除标签TAG OK"
    ...
    @staticmethod
    def maintain_start(parent = None):
        comsetting_dialog = ComMaintainer(parent)
